...
namespace Es.InkPainter
{
    public static class Math
    {
        ...
        public static Vector2 TextureCoordinateCalculation(Vector3 p, Vector3 t1, Vector2 t1UV, Vector3 t2,
            Vector2 t2UV, Vector3 t3, Vector2 t3UV, Matrix4x4 transformMatrix)
        {
            var p1_p = transformMatrix * new Vector4(t1.x, t1.y, t1.z, 1);
            var p2_p = transformMatrix * new Vector4(t2.x, t2.y, t2.z, 1);
            var p3_p = transformMatrix * new Vector4(t3.x, t3.y, t3.z, 1);
            var p_p = transformMatrix * new Vector4(p.x, p.y, p.z, 1);
            var p1_n = new Vector2(p1_p.x, p1_p.y) / p1_p.w;
            var p2_n = new Vector2(p2_p.x, p2_p.y) / p2_p.w;
            var p3_n = new Vector2(p3_p.x, p3_p.y) / p3_p.w;
            var p_n = new Vector2(p_p.x, p_p.y) / p_p.w;
        ...
        public static Vector3[] GetNearestVerticesTriangle(Vector3 p, Vector3[] vertices, int[] triangles)
        {
            ...
            var nearestIndex = triangles[0];
            var nearestDistance = Vector3.Distance(vertices[nearestIndex], p);

            for (var i = 0; i < vertices.Length; ++i)
            {
                var distance = Vector3.Distance(vertices[i], p);
