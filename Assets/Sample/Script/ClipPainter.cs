...
namespace Es.InkPainter.Sample
{
    ...
    {
        [SerializeField] private bool grab = true;

        [SerializeField] private Brush brush = null;

        [SerializeField] private GrabArea.GrabTextureWrapMode wrapMode = GrabArea.GrabTextureWrapMode.Repeat;
        ...
        private void Update()
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(ray, out hitInfo))
                {
                    ...
                    if (d != null && !grab) d.Paint(brush, hitInfo);
                    if (grab)
                    {
                        GrabArea.Clip(brush.BrushTexture, brush.Scale,
                            hitInfo.transform.GetComponent<MeshRenderer>().sharedMaterial.mainTexture,
                            hitInfo.textureCoord, brush.RotateAngle, wrapMode, t);
