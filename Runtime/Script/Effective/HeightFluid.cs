...
namespace Es.InkPainter.Effective
{
    ...
    public class HeightFluid : MonoBehaviour
    {
        ...
        [SerializeField] private bool useMainTextureFluid = true;

        [SerializeField] private bool useNormalMapFluid = true;

        [SerializeField] private int createTextureSize = 1024;

        [SerializeField] private ColorSynthesis colorSynthesis = ColorSynthesis.Overwrite;

        [SerializeField] [Range(0, 1)] private float alpha = 1f;

        [SerializeField] private Vector2 flowDirection = Vector2.up;

        [SerializeField] [Range(0, 1)] private float flowingForce = 1;

        [SerializeField] [Range(0.1f, 10f)] private float easeOfDripping = 1.0f;

        [SerializeField] [Range(1f, 0f)] private float influenceOfNormal = 1;

        [SerializeField] [Range(0.01f, 1)] private float horizontalSpread = 0.01f;

        [SerializeField] private float normalScaleFactor = 1;

        [SerializeField] [Range(0.001f, 0.999f)]
        private float AdhesionBorder = 0.01f;

        [SerializeField] private bool performanceOptimize = true;

        [SerializeField] [Range(0.01f, 10f)] private float fluidProcessStopTime = 5f;
        ...
        {
            var tmp = RenderTexture.GetTemporary(texture.width, texture.height, 0, RenderTextureFormat.ARGB32,
        ...
        private void OnWillRenderObject()
        {
            ...
            foreach (var set in canvas.PaintDatas)
            {
                ...
                var heightTmp = RenderTexture.GetTemporary(heightPaint.width, heightPaint.height, 0,
                    RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
                ...
                if (useMainTextureFluid)
                {
                    ...
                    var mainTmp = RenderTexture.GetTemporary(mainPaint.width, mainPaint.height, 0,
                        RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
                ...
                if (useNormalMapFluid)
                {
                    ...
                    var normalTmp = RenderTexture.GetTemporary(normalPaint.width, normalPaint.height, 0,
                        RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
